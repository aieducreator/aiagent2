...
### 2. LangGraph 상태 정의: TypedDict 클래스 함수 상속
...
def summarize_result(state: AgentState) -> Dict:
    ...
    summary = f"도구 실행 결과 '{tool_result}'를 요약했습니다."    
    print(f"\n결과 요약: {summary}")
...
print(f"\n최종 상태 메시지 (TypedDict): {final_state_example['messages']}")
print('-'*80)
print(f"최종 도구 출력 (TypedDict): {final_state_example['tool_output']}")