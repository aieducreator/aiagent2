...
async def retriever_node(state: AgentState) -> Dict[str, Any]:
    print("--- 자료 조사 담당: 관련 매뉴얼 검색 ---")
    question = state.messages[-1].content
    retrieved_docs = await retriever.ainvoke(question)
    context_text = "\n\n---\n\n".join([doc.page_content for doc in retrieved_docs])
    return {"context": context_text, "next": "answer_agent"}

async def answer_node(state: AgentState) -> Dict[str, Any]:
    print("--- 답변 생성 담당: 최종 답변 작성 ---")
    template = """
    당신은 '자료 조사 담당'이 찾아준 '내부 매뉴얼'을 바탕으로, 고객의 '질문'에 답변하는 고객 지원 전문가입니다.
    내부 매뉴얼: {context}
    고객 질문: {question}
    """
    prompt = ChatPromptTemplate.from_template(template)
    chain = prompt | llm | StrOutputParser()
    response = await chain.ainvoke({"context": state.context, "question": state.messages[-1].content})
...
# 7-3 그래프의 시작점을 'retriever' 노드로 설정
workflow.set_entry_point("retriever")

# 7-4 엣지 추가
workflow.add_conditional_edges("retriever", router)
workflow.add_conditional_edges("answer_agent", router, {"end": END})
...
async def main():
    ...
    async with AsyncSqliteSaver.from_conn_string(db_file) as memory:
        ...
        print("안녕하세요! 코드크래프터스 AI 멀티 에이전트입니다. 무엇을 도와드릴까요? (종료: exit)")
        print('\n')

        # 각 대화를 식별하기 위한 고유 ID 설정
        config = {"configurable": {"thread_id": "user-5678"}}
